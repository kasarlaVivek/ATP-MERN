...
userApp.put('/user/:id', async (req, res) => {
    let reqUser = await userModel.findOneAndUpdate({ _id: req.params.id, status: true }, { $set: { ...req.body } }, { new: true });
    !reqUser ? res.status(404).json({ message: "User not found" }) : res.status(201).json({ message: "user updated", payload: reqUser });
})
